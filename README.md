...
공식 사용량 화면으로 이동하는 버튼을 제공합니다. Cloudflare 현재 계정의 사용량 기반 알림 제품 목록은 비어 있어 한도 정책 저장은 완료하지 못했습니다. Vercel 기본 이메일/배포 실패 알림은 켜져 있으나 기존 계정 이메일로 수신하며, 별도 장애 알림 주소로의 전달은 미설정입니다. SMTP 연결 완료를 사용량 한도 경보 전체 완료로 해석하지 않습니다.
